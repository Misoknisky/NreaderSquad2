...
def get_impossible(is_impossible):
    if is_impossible:
        return [0.0,1.0]
    return [1.0,0.0]
def process_file(filename, data_type, word_counter, char_counter):
    ...
    with open(filename, "r") as fh:
        ...
        for article in tqdm(source["data"]):#只有一个data字段,存放所有的数据,对应多个paragraphs
            for para in article["paragraphs"]:# 一个paragraphs字段对应多个context,每次处理一个context
                context = para["context"].replace("''", '" ').replace("``", '" ')#一个context对应一篇文章, 多个问题
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    ...
    for example in tqdm(examples):
        ...
        impossible=np.asarray(example["is_impossible"], dtype=np.float32)
        def _get_word(word):
        ...
        record = tf.train.Example(features=tf.train.Features(feature={
                                  ...
                                  "is_impossible":tf.train.Feature(bytes_list=tf.train.BytesList(value=[impossible.tostring()]))
                                  }))
...
def prepro(config):
    word_counter, char_counter = Counter(), Counter()#统计单词个数
    #加载数据
    ...
    word_emb_mat, word2idx_dict = get_embedding(word_counter, "word", emb_file=word_emb_file,size=config.glove_word_size, vec_size=config.glove_dim, token2idx_dict=word2idx_dict)

    char2idx_dict = None
    if os.path.isfile(config.char2idx_file):
        with open(config.char2idx_file, "r") as fh:
            char2idx_dict = json.load(fh)
    char_emb_mat, char2idx_dict = get_embedding(char_counter, "char", emb_file=char_emb_file, size=char_emb_size, vec_size=char_emb_dim, token2idx_dict=char2idx_dict)
